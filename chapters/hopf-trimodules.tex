...
\begin{reptheorem}{finitetrihopfreconstruction}
  ...
  where the \(\tetramodfd[B]\)\hyp{}module structure on the right-hand side is extended from the category of free \(A\)\hyp{}modules.
...
\begin{proof}
  ...
  we show that the induced arrow
  \[
...
\begin{theorem}\label{finitetrihopfreconstruction}
  ...
  is extended from the free left \(C\)\hyp{}contramodules.
\end{theorem}
\begin{proof}
...
The standard comultiplication on \(\Bbbk[S]\), in which \(e\) and \(s\) are group-like, turns \(\Bbbk[S]\) into a bialgebra.
...
\begin{proof}
  ...
  as both of the coactions are given by that of \(B\) on the respective factor.
...
\begin{proof}
  ...
  \]
  where we have denoted the counit \(\varepsilon\) of \(B\) with a small white dot,
...
\begin{definition}\label{bestoftimes}
  ...
  \]
  where we extend to morphisms in the natural way.
...
\begin{remark}
  ...
  This is merely to maintain consistency with \cref{sec:hopf-trimodules-lax-mod-funs},
...
\begin{proposition}\label{EmperorDaibazaal}
  ...
  its right vertical arrow is the equivalence of \cref{thm:hopf-trimodules-are-lax-monoidal-functors},
...
\begin{proof}
  ...
  where the lower part is included for clarity:
  \[
    \begin{mytikzcd}[ampersand replacement=\&]
...
\begin{proof}
  ...
  Recall that any \(T\)\hyp{}algebra \((x, \alpha) \in \cat{C}^T\)
