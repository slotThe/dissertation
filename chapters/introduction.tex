...
there exist \(\cat{C}\)\hyp{}modules categories from which it is impossible to reconstruct an algebra object in \(\cat{C}\).
...
Being an amalgamate of the preliminaries of all articles,
...
endowed with Day convolution as its tensor product.
...
As it turns out, R-matrices on a suitable monad correspond to duoidal structures its Eilenberg–Moore category.
...
\begin{reptheorem}{extendingalways,strongembeddingalways}
  ...
  Then there exists an essentially uniquely \(\cat{C}\)\hyp{}module structure on \(\cat{M}^{T}\),
