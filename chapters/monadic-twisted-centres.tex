...
\begin{definition}\label{def: central_bimonad}
  ...
  We denote its Eilenberg–Moore category by \(\cat{C}^{\mathfrak{D}}\).
\end{definition}
...
\begin{remark}
  The identifications of \(\cat{C}^{Z_B}\) and \(\cat{C}^{Z_L}\) with the generalised Drinfeld centre
...
\begin{proof}
  ...
  \[
    ...
    \ZCat[L]{C}.
  \]
  For any \(m \in \ZCat[L]{C}\) and \(x \in \cat{C}^{Z_B}\) we have
  \[
    K^{Z_L} (m \bract x)
    =
...
\begin{lemma}\label{lem: cross_products_and_oplax_monoidal_actions}
  The oplax monoidal right action \(\alpha \from LB \nt L\) induces an oplax monoidal action of \(B \rtimes H\) on \(L \rtimes H\) by
  \[
...
\begin{remark}\label{blackstar}
  ...
  implies that we obtain a comodule monad \(D(L,H) \defeq Z_L \rtimes H\) over \(D(B, H) \defeq Z_B \rtimes H\).
...
Classically, these consist of a group-like and a character of a Hopf algebra
