...
\begin{proof}
  ...
  \(\cat{A}^T\) is finitely cocomplete.
...
\begin{proposition}\label{ModuleEMSemisimple}
  ...
  The equivalence \(\cat{M}^T \simeq \cat{M}^S\) of \cref{semisimpleleftandright} is a \(\cat{C}\)\hyp{}module equivalence,
...
\begin{definition}\label{representablemultiactegory}
  ...
  we say that \(\mathbf{M}\) is \emph{representable with respect to \(\mathsf{R}\)},
...
\begin{proof}
  ...
  The coequaliser of Diagram~\eqref{unicharacterize} defines a pre-universal map in \(\mathbf{M}^{\varphi}\) with domain \((x_{1},\ldots,x_{n};m')\), by Lemma~\ref{multiuni}.
...
\begin{definition}
  ...
  a \emph{\(T\)\hyp{}\(S\)\hyp{}biact functor} is a \(T\)\hyp{}\(S\)\hyp{}bimodule object in \(\mathsf{Rex}(\cat{A},\cat{A})\).
\end{definition}
