...
\begin{theorem}\label{thm:SmallBeck}
  Let \(\adj{F}{U}{\cat{A}}{\cat{B}}\) be an adjunction between abelian categories.
  Then \(U\) is right exact and reflects zero objects\note{%
    A functor \(F\) \emph{reflects zero objects} if \(Fx \cong 0 \implies x \cong 0\).%
  }
  if and only if the comparison functor \(K \from \cat{B} \to \cat{A}^{UF}\) is an equivalence.
  Likewise, \(F\) is left exact and reflects zero objects
  if and only if the comparison functor \(K\from \cat{A} \to \cat{B}^{FU}\) is an equivalence.
\end{theorem}

That \cref{thm:SmallBeck} is equivalent to the general monadicity theorem
in the case of abelian categories
is explained in~\cite[Section~4.1]{ben-zvi18:integ}.
