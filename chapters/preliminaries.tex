...
and 2-natural transformations as 2-cells.
...
\begin{definition}\label{def:distributive-law}
  Let \(T\) and \(S\) be two monads on a category \(\cat{C}\).
...
\begin{example}[{\cite{Hajac2004a}}]\label{ex:ayd-def-as-modules-over-yd}
  ...
  The category  of anti\hyp{}Yetter–Drinfeld modules does not form a monoidal category, but rather a \(\cat{C}\)\hyp{}module category over the Yetter–Drinfeld modules.
...
\begin{example}\label{ex:regularend}
  For an object \(x\) in a (non-strict) monoidal category \(\cat{C}\),
...
They were introduced by Joyal and Street in~\cite{joyal85:braided,joyal86:braided,Joyal1993},
...
and call \(\adj{\cohom{m, \blank}}{\blank \lact m}{\mm}{\cc}\) the \emph{internal cohom--tensor adjunction}.
...
The next result links the invertibility of this morphism to conditions that \(\cat{C}\) is rigid monoidal;
...
It follows that the right evaluation and coevaluation of an invertible object are isomorphisms as well.
Further, tensor products and duals of invertible objects are invertible,
such that the full subcategory \(\mathsf{Inv}(\cat{C})\) of invertible objects of \(\cat{C}\) is rigid monoidal.
...
\begin{remark}
  A left closed monoidal category such that for every \(x \in \cat{C}\),
...
We will study these questions in greater detail in \cref{sec:trep-categories};
...
\end{theorem}
\begin{proof}
  Suppose that \(\cat{C}\) is a rigid strict monoidal category.
  ...
  \[
    ...
    \end{cases}
  \]
  The right dual of \(x\) is defined similarly as \(\rd{x} \defeq (x_i^{n_i-1}, \dots, x_1^{n_1-1})\).
...
\begin{proposition}[{\cite[Remark~4]{ostrik03:modul-hopf},~\cite[Lemma~2.10]{douglas19}}]\label{prop:lax-is-strong-when-rigid}
  If \(\cat{C}\) is a left rigid monoidal category, then every lax \(\cat{C}\)\hyp{}module functor is strong.
...
and write \(\kvect\) for the full subcategory of finite\hyp{}dimensional \(\Bbbk\)\hyp{}vector spaces.\note{%
  ...
  inside of a larger category of all objects,
...
\begin{example}\label{ex:bicategory-of-bimodules}
  ...
  such that it is right exact in both variables.
...
A more detailed account is given in~\cite[Chapter~12]{johnson-yau2021:TwoDimensionalCategories},
...
\begin{example}\label{MonoidalBicatProf}
  ...
  Further, \(\Prof_{\Bbbk}\) is monoidal when endowed with the monoidal structure \(\otimes_{\Bbbk}\).
...
\begin{example}\label{ex:graphical-opmonoidal-adjunctions}
  ...
  are oplax monoidal natural transformations are
...
\begin{definition}\label{def:coend-vect}
  ...
  The \emph{coend} of a \(\Bbbk\)\hyp{}linear functor \(P\from \cat{C}^{\op} \otimes_{\Bbbk} \cat{C} \to \kVect\) is the following coequaliser in \(\kVect\):
  \[
    \begin{mytikzcd}[ampersand replacement=\&]
...
\begin{remark}
  ...
  we implicitly equip it with the convolution tensor product.
...
By~\cite[Theorem~6.2]{lopez11:pseud-kz}, this yields a pseudoclosed structure on the \(2\)\hyp{}category \(\BiCat{Cat}_{\Cocts{\Phi_{c}}}\)
