...
\begin{reptheorem}{prop:2-groups-tensor-product}
  Let \(G\) and \(H\) be finite groups, and suppose
  Suppose that
  \[
...
\end{align*}
for the units and counits of the correspondig adjunctions.
...
in practise it still behaves very similarly to how a property would,
...
As an intermediate step in the construction of \(\cat{W}\), we define a strict monoidal category \(\cat{V}\), which has the same generators as \(\cat{W}\) but less relations.
...
Examples of such categories of functors are abound,
and include modules over path algebras, Mackey functors, and
group-graded representations arising from crossed modules;
we shall more closely investigate these examples in \cref{sec:mky-2-grps}.
...
\begin{remark}\label{rem:day-morphism-invertible}
  ...
  left and right duals coincide and  we can chose the ``quantum trace'' of \cref{eq:day-canonical-candidate}
...
\begin{proof}
  ...
  via the multiplication following multiplication, for \(f \in \cat{C}(a, b)\), \(g \in \cat{C}(c, d)\):
  \[
    f \cdot g \defeq
    \begin{cases*}
      g \circ f, & \text{if } b = c, \\
...
\begin{definition}\label{def:Boolean-algebra}
  ...
  \begin{equation}\label{eq:distributivity-equation}
    ...
    a \lor (b \land c) = (a \land b) \lor (a \land c),
  \end{equation}
...
\begin{example}\hfill
  \begin{itemize}
    ...
    In case \(R\) is furthermore Artinean, this is equivalent to it being semisimple.
...
any representation of \(G\) defines one, see~\cite[Example~53.1]{thevenaz1995:G-algebrasModularRepresentation} as well as~\cite[Propostion~10.1]{panchadcharam07:mackey}.
...
The following results gives an alternative proof for the sketched argument
...
\(\overline{\alpha} \from Q \defeq G/N \to \mathrm{Aut}(L)\), with
\[
...
the surjective homomorphisms of groups \(t\from H \to K\).
...
\begin{notation}
  ...
  Given some \(M\in \mathsf{rep}_{\cat{G}}(L)\), we write \(M_q\) for its the homogeneous component of degree \(q\in Q\).
